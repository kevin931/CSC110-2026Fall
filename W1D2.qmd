...
### Detour: The Pipe Operator

In tidyverse, we often start with a dataset and perform an operation on it. This is where the pipe operator comes in. Let's start with a simple example with a new dataset called `mtcars` :

```{r}

```

Annoyingly, there are two notations:

```{r}

```

Which one to use is up to you! In the book you will see the latter. You read the pipe `|>` as "Take X and do Y". In practice, the pipe takes X and put it as the first argument of function Y.
