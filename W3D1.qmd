---
title: "W3D1: Filtering and arranging data"
format: typst
---
...
In this class, we will use the pipe operator extensively. There are two pipe operators you can use in R:
...
Pipes take the output of one function and pass it to the input of the subsequent function.

When you put this all together, you get a **data pipeline**.

We are going to use a range of functions as part of the pipeline:

- Use `filter()` to only get the observations we need
- Use `select()` to only get the variables we need
- Use `mutate()` to calculate new variables
- Use `summarize()` to aggregate the data
- Use `arrange()` to sort the final output
...
Simply writing `df <- myData` will produce a new data frame, but it will not display the output. You need to call the data separately to produce some outputs.
:::
...
Quick side note here: we can use Mermaid, a JavaScript-based markup language, to create diagrams that visualize the flow of our data through dplyr pipelines.

What we just did in the first few code blocks can be visualized like so:
...
::: callout-tip
**Pro tip**: The site [mermaid.live](https://mermaid.live/){target="_blank"} provides a free interactive tool for writing Mermaid diagrams and looking at their output. Once you've got your diagram the way you want it to look, you can copy-paste it into a Quarto code block like I've done in the two examples above, and it should render in your output!
