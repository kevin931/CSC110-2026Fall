...
- How does W.E.B. Du Bois's work innovative?

- What do you observe about his work?
...
Note the following: - 2,000 rows - 13 variables with a mixture of numeric (`<dbl>`) and categorical (`<chr>`) data types - Some values are marked `NA`, which indicates the absence of data

# Types of variables

- **Categorical variables** are those that can take only one of a small set of values[^1]. These are typically represented as text (character) data, and are attributes like names, places, groups, etc. Also known as qualitative data or dimensional data. Below are some examples:

  - Example 1:

  - Example 2:

- **Numerical variables** are those that are represented by numbers that we can add, subtract, or aggregate in some fashion[^2]. Think of ages, weights, grade-point averages, etc. Also known as quantitative data, numerical variables can be discrete or continuous:
...
[^1]: See [R4DS 1.4.1](https://r4ds.hadley.nz/data-visualize.html#a-categorical-variable){target="_blank"}

[^2]: See [R4DS 1.4.2](https://r4ds.hadley.nz/data-visualize.html#a-numerical-variable){target="_blank"}
...
**Histograms** appear similar to bar charts, but there's a major distinction between them: histograms visualize the distribution of a *numerical* variable along the X axis, whereas a bar chart puts a categorical variable on the X axis.
